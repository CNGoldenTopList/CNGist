...
export type SiteConfig = { providers: AuthProvider[]; mailAvailable: boolean; bindingArticleUrl?: string };
...
export function fetchSiteConfig(): Promise<SiteConfig> {
  ...
  inflight = api.get<SiteConfig>("/api/config").then(({ ok, data }) => {
    if (ok) state.value = { providers: data.providers ?? [], mailAvailable: Boolean(data.mailAvailable), bindingArticleUrl: data.bindingArticleUrl };
    inflight = null;
