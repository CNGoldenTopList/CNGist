...
export type SiteConfig = { providers: AuthProvider[]; mailAvailable: boolean; bindingArticleUrl?: string; icpNumber?: string; developers: SitePerson[]; sponsorship: string; administrators: SitePerson[] };

const EMPTY: SiteConfig = { providers: [], mailAvailable: false, developers: [], sponsorship: "", administrators: [] };
...
export function fetchSiteConfig(): Promise<SiteConfig> {
  ...
  inflight = api.get<SiteConfig>("/api/config").then(({ ok, data }) => {
    if (ok) state.value = { providers: data.providers ?? [], mailAvailable: Boolean(data.mailAvailable), bindingArticleUrl: data.bindingArticleUrl, icpNumber: data.icpNumber, developers: data.developers ?? [], sponsorship: data.sponsorship ?? "", administrators: data.administrators ?? [] };
    inflight = null;
